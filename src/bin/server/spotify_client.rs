...
impl SpotifyClient {
    ...
    pub(crate) fn new(server_redirect_uri: &str) -> Self {
        ...
        let ClientConfig {
            client_id,
            client_secret: _,
        } = Self::load_config_from_env().expect("Failed to load initial client configuration");
        let cached_access_token = Arc::new(Mutex::new(CachedAccessToken {
            code_verifier: None,
            access_token: None,
            refresh_token: None,
        ...
        let refresh_thread =
            Self::build_refresh_thread(cached_access_token.clone(), client_id.clone(), shutdown_rx);
    ...
    fn load_config_from_env() -> Result<ClientConfig, error::ServerError> {
        let client_id = std::env::var("SPOTIFY_CLIENT_ID")
            .map_err(|env_err| error::ServerError::ConfigError(env_err.to_string()))?;
                    ...
                    Err(RecvTimeoutError::Timeout) => {
                        ...
                        if needs_refresh {
                            ...
                            if cached_token_guard.refresh_token.is_none() {
                                println!(
                                    "No refresh token available, cannot refresh Spotify access token"
                                );
                                continue;
                            }
                            let request_form = [
                                ...
                                (
                                    "refresh_token",
                                    cached_token_guard.refresh_token.as_ref().unwrap(),
                                ), // Safe to unwrap since we already checked for empty earlier
                                ("client_id", &client_id),
                            ];
                            let token_refresh_url = Self::get_spotify_token_url();
                            if let Err(error) = token_refresh_url {
                                println!("Error building Spotify token refresh URL: {}", error);
                                continue;
                            }
    ...
    fn get_spotify_auth_url(&self) -> Result<(String, String), error::ServerError> {
        match verification_util::build_code_challenge() {
            Ok((code_verifier, code_challenge)) => Url::parse(&format!(
                "https://{}{}",
                Self::SPOTIFY_AUTH_BASE_URL,
    ...
    pub(crate) fn start_client_auth(&mut self) {
        ...
        let (spotify_auth_url, code_verifier) = self
            .get_spotify_auth_url()
            .unwrap_or_else(|_| panic!("Failed to generate Spotify authorization URL"));
        self.cached_access_token.lock().unwrap().code_verifier = Some(code_verifier);

        println!("Authorize with Spotify: {}", spotify_auth_url);
    }

    pub(crate) fn handle_auth_callback(
        &mut self,
        auth_params: Query<SpotifyAuthCallbackParams>,
    ) -> Result<(), error::ServerError> {
        match Self::get_spotify_token_url() {
            Ok(token_url) => {
                // Clone the code_verifier out of the mutex so it lives long enough
                let mut cached_token_guard = self.cached_access_token.lock().unwrap();
                let code_verifier = cached_token_guard.code_verifier.clone().ok_or(
                    error::ServerError::TokenError(
                        "Code verifier not found in cached access token".to_string(),
                    ),
                )?;
                let request_form = [
                ...
                ureq::post(token_url)
                    ...
                    .map(|mut response| {
                        if let Ok(access_token) = response
                            .body_mut()
                            .read_json::<payload::SpotifyAccessTokenResponseBody>(
                        ) {
                            // Ensure the code verifier is the one that was sent to the Token generation URL
                            cached_token_guard.code_verifier.replace(code_verifier);
                            cached_token_guard.access_token =
                                Some(access_token.access_token.clone());
                            cached_token_guard.refresh_token =
                                Some(access_token.refresh_token.clone());
                            cached_token_guard.expires_at = SystemTime::now()
                                + Duration::from_secs(access_token.expires_in - 60);
                            println!(
                                "Received a Spotify access token expiring in {} seconds",
                                access_token.expires_in
                            );
                        }
                    })
                    .map_err(|error| {
                        error::ServerError::AuthError(format!(
                            "Error processing access token response: {}",
                            error
                        ))
                    })
            }
    ...
    pub(crate) fn list_playlists(&self) -> Result<Vec<String>, error::ServerError> {
        let access_token = self
            ...
            .access_token
            .clone()
            .ok_or(error::ServerError::TokenError(
                "No access token available".to_string(),
            ))?;

        let playlists_url = Url::parse(&format!(
            "https://{}/v1/me/playlists",
        ...
        ureq::get(playlists_url.as_str())
            ...
            .map(|mut response| {
                if let Ok(json_response) = response
                    .body_mut()
                    .read_json::<payload::SpotifyListPlaylistsResponse>()
                {
            ...
            .map_err(|error| {
                error::ServerError::ClientError(format!("Error fetching playlists: {}", error))
            })
    }
}
