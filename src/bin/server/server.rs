...
use crate::{
    spotify_client::{SpotifyAuthCallbackParams, SpotifyClient},
    types::error,
};
...
impl Server {
    ...
    fn load_config_from_env(load_env_file: bool) -> ServerConfig {
        ...
        let server_port_str = std::env::var(Self::SERVER_CONFIG_PORT_KEY).unwrap_or_else(|_| {
            panic!(
                "{} must be set in environment variables",
                Self::SERVER_CONFIG_PORT_KEY
            )
        });
        let server_port = server_port_str
            .parse::<u16>()
            .unwrap_or_else(|_| panic!("{} must be a valid u16", Self::SERVER_CONFIG_PORT_KEY));
    ...
    pub(crate) async fn run(&self) {
        ...
        let listener = TcpListener::bind(format!("127.0.0.1:{}", self.server_config.server_port))
            .await
    ...
    fn get_configured_router(&self) -> Router {
        ...
        let router: Router = Router::new()
            ...
            .route(
                ...
                get({
                    ...
                    async move |auth_params: Query<SpotifyAuthCallbackParams>| {
                        if let Err(callback_err) = spotify_client
                            .lock()
                            .await
                            .handle_auth_callback(auth_params)
            ...
            .route(
                Self::LIST_PLAYLISTS_PATH,
                get({
                    let spotify_client = Arc::clone(&self.spotify_client);
                    async move || match spotify_client.lock().await.list_playlists() {
                        Ok(playlists) => {
                            println!("Playlists: {:?}", playlists);
                        }
                        Err(list_playlists_err) => {
                            eprintln!(
                                "Error occurred while listing playlists: {}",
                                list_playlists_err
                            );
                        }
                    }
